...
{
    //Making a copy of the arrays and using this 
    //so that we can retain original data in the original arrays 
    //If the resource requests are dependent, then 
    //there is no need of the copies of the original array 
    int allocation2[100][100],max2[100][100],need2[100][100],available2[100];
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
    ...
    for(int i=0;i<n;i++)
    {
        if(i==rp) //For the process requesting resources 
        {
            ...
            for(int j=0;j<m;j++)
            {
                ...
                if(request[j]>need2[i][j] and request[j]>available2[j])
                {
                    //Set flag to 1
        ...
        for(int i=0;i<n;i++)
        {
            if(i==rp) //For the process requesting resources 
            {
                ...
                for(int j=0;j<m;j++)
                {
                    ...
                    available2[j]=available2[j]-request[j];
                    allocation2[i][j]=allocation2[i][j]+request[j];
                    need2[i][j]=need2[i][j]-request[j];
                }
            }
        }
        //Call safety algorithm to check for safety if request granted 
        result=safety(n,m,allocation2,need2,available2);
...
int main() //Main function 
{
    ...
    if(x==true)
    {
        ...
        if(ch=='Y')
        {
            cout<<"Enter the number of processes for resource request : "<<endl;
            cout<<"All the resource requests are independent!"<<endl;
            cin>>nrr; //Number of independent resource requests 
            for(int i=0;i<nrr;i++)
            {
                //Function call for resource request algorithm 
